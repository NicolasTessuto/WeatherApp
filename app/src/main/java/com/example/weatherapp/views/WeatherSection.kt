...
fun WeatherSection(weatherResponse: WeatherResult) {
    ...
    subTitle = if (dateVal == 0) Const.LOADING
    ...
    weatherResponse.weather.let {
        if (it!!.size > 0) {
            description = if (it[0].description == null) Const.LOADING else it[0].description!!
            icon = if (it[0].icon == null) Const.LOADING else it[0].icon!!
        }
    }
    ...
    weatherResponse.wind.let {
        wind = if (it == null) Const.LOADING else "${it.speed}"
    }

    var clouds = ""
    weatherResponse.clouds.let {
        clouds = if (it == null) Const.LOADING else "${it.all}"
    }

    var snow = ""
    weatherResponse.snow.let {
        snow = if (it!!.d1h == null) Const.NA else "${it.d1h}"
    }
...
fun WeatherTitleSection(text: String, subText: String, fontSize: TextUnit) {
    Column(
        ...
        Text(subText, fontSize = 26.sp, color = Color.White)
    }
}
